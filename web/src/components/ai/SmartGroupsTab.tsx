...
import { AlertTriangle, ChevronDown, ChevronRight, Sparkles, Wrench, XCircle, Zap } from "lucide-react";
...
export default function SmartGroupsTab({
  ...
  const [triagingGroup, setTriagingGroup] = useState<string | null>(null);
  ...
  const aiTriageGroup = useMutation({
    mutationFn: ({ groupId, fingerprints }: { groupId: string; fingerprints: string[] }) => {
      setTriagingGroup(groupId);
      return apiFetch(`/api/v1/scans/${scanId}/ai-triage`, {
        method: "POST",
    ...
    onSuccess: (_, { fingerprints }) => {
      ...
      setTriagingGroup(null);
      toast.success(`AI triage started for ${fingerprints.length} findings — check AI Triage tab`);
    },
    onError: (e: Error) => { setTriagingGroup(null); toast.error(e.message); },
  });
  ...
  return (
    <div className="space-y-4">
            ...
            >
              ...
              {isExp && (
                <div className="border-t border-edge-soft px-5 py-4 space-y-4">
                  ...
                  {/* AI Triage this group */}
                  <div className="flex flex-wrap items-center gap-2 border-t border-edge-soft pt-3">
                    <span className="self-center text-xs text-content-faint">AI Triage:</span>
                    <button
                      type="button"
                      onClick={() => aiTriageGroup.mutate({ groupId: g.group_id, fingerprints: g.finding_fingerprints })}
                      disabled={triagingGroup === g.group_id}
                      className="flex items-center gap-1.5 rounded-lg bg-gradient-to-r from-purple-600 to-blue-600 px-3 py-1 text-xs font-semibold text-white hover:from-purple-500 hover:to-blue-500 disabled:opacity-50"
                    >
                      <Zap className="h-3 w-3" />
                      {triagingGroup === g.group_id ? "Starting…" : `AI Triage ${g.finding_count} findings`}
                    </button>
                  </div>

                  {/* Manual bulk triage */}
                  <div className="flex flex-wrap gap-2 border-t border-edge-soft pt-3">
                    <span className="self-center text-xs text-content-faint">Manual triage all:</span>
      ...
      <p className="text-right text-xs text-content-faint">Generated by AI · Review before bulk-triaging</p>
    </div>
  );
}
